...
def level_pair(raw_level: any, default: tuple[int, int]) -> tuple[int, int]:
    """
    Normalize refinement level input to a pair.
    
    Args:
        raw_level: Input level specification (int, float, or tuple/list).
        default: Default level pair if input is None.
        
    Returns:
        tuple[int, int]: Normalized (min_level, max_level).
    """
...
def refinement_surfaces(
    ...
    Args:
        surface_dict: Dictionary of surface refinement settings.
        default_patch: Default patch name if none specified.
        default_levels: Default (min_level, max_level) pair.

    Returns:
        str: The refinementSurfaces section.
    """
    if not surface_dict:
    ...
    for patch_name, settings in surface_dict.items():
        level_min, level_max = level_pair(settings.get("Level"), default_levels)
...
def box_geometry(
    ...
    Args:
        ...
        name: Name of the box region.
        
    Returns:
        str: Searchable box geometry entry.
    """
    return f"""    {name}
    {{
        type searchableBox;
...
def sphere_geometry(
    ...
    """
    Create sphere geometry entry.

    Args:
        center: Center coordinates (x, y, z).
        radius: Sphere radius.
        name: Name of the sphere region.

    Returns:
        str: Sphere geometry entry.
    """
    return f"""    {name}
    {{
...
def refinement_regions_func(
    ...
    Args:
        ...
        region_dict: Dictionary of region refinement settings.
        default_levels: Default (min_level, max_level) pair.
        
    Returns:
        tuple[str, str]: Geometry entries and refinementRegions section.
    """
    if not region_dict:
    ...
    for name, settings in region_dict.items():
        ...
        if ref_type == "sphere":
            ...
            geometry_entries.append(sphere_geometry(center, radius, name))
        else:
            ...
            geometry_entries.append(box_geometry(min_corner, max_corner, name))
...
def feature_section(level: int, file_name: str = "airfoil.eMesh") -> str:
    """
    Build the features section if requested.
    
    Args:
        level: Feature refinement level.
        file_name: Name of the feature edge file.
        
    Returns:
        str: The features section.
    """
...
def layers_section(layer_dict: dict, add_layers_settings: dict) -> tuple[str, bool]:
    """
    Build layers subsection and flag whether layers are enabled.
    
    Args:
        layer_dict: Dictionary of boundary layer settings.
        add_layers_settings: Additional settings for addLayersControls.
        
    Returns:
        tuple[str, bool]: The addLayersControls section and enabled flag.
    """
    layer_entries = []
...
{{
    relativeSizes {bool_to_foam(relative_sizes)};
    layers
    {{
{layers_block}
    }}
...
def generate_snappy_hex_mesh_dict(
    ...
    Args:
        airfoil: Airfoil object with geometry data
        setup: Settings object containing simulation configuration
        location_in_mesh: Point inside the mesh domain
        snappy_settings: SnappyHexMesh settings dictionary
        surface_refinement_dict: Surface refinement settings
        region_refinement_dict: Region/object refinement settings
        boundary_layers_dict: Boundary layer settings

    Returns:
        str: Complete snappyHexMeshDict file content
    """
    ...
    geometry_extra, refinement_regions = refinement_regions_func(
        airfoil=airfoil,
...
{{
    ...
    features_section = feature_section(
        level=castellated.get("FeatureRefinementLevel", 0)
    )
    refinement_surfaces_content = refinement_surfaces(
    ...
    add_layers_section, add_layers_enabled = layers_section(
    ...
    mesh_quality = {**quality_defaults, **{k: v for k,
                                           v in mq_user.items() 
                                           if k not in ("relaxed", "Relaxed")}}
...
snap            true;
addLayers       {bool_to_foam(add_layers_enabled)};
...
{refinement_regions}
    ...
    allowFreeStandingZoneFaces {bool_to_foam(castellated['AllowFreeStandingZoneFaces'])};
}}
